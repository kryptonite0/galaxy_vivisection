...
def mbh_vs_mass_sph_agn():
	...
	bulge[simplemorphtype=='Sp'] = 1
	bulge[simplemorphtype=='S0/Sp'] = 1
	...
	print 'M_BH/M_*,sph =', np.median(10**log_mbh[earlytype==1]/10**log_mass_sph[earlytype==1]), '+-', (np.std(10**log_mbh[earlytype==1]/10**log_mass_sph[earlytype==1]))/(len(log_mass_sph[earlytype==1])-1)
       ...
       #
       #### produce .dat file
       #datfileName = '/Users/gsavorgnan/galaxy_vivisection/data/marconi_fit/mbh_vs_mass_sph_early.dat'
       #datfile = open(datfileName, 'w')
       #datfile.write('# logMassSph err_logMassSph logMassBH err_logMassBH \n')
       #for logMassSph, err_logMassSph, logMassBH, err_logMassBH in zip(log_mass_sph[earlytype==1]-np.average(log_mass_sph[earlytype==1]),
       #	0.5*(perr_log_mass_sph[earlytype==1] + merr_log_mass_sph[earlytype==1]),
       #	log_mbh[earlytype==1],0.5*(merr_log_mbh[earlytype==1] + perr_log_mbh[earlytype==1])):
	...
	ax.errorbar(mass_sph[morph_core==u'E_0'], mbh[morph_core==u'E_0'], 
        ...
        ax.errorbar(mass_sph[morph_core=='Sp_0'], mbh[morph_core=='Sp_0'], 
        	xerr=[merr_mass_sph[morph_core=='Sp_0'],perr_mass_sph[morph_core=='Sp_0']], 
...
def mbh_vs_mass_sph():
	...
	morph_coreList = []
       ...
       #datfile.close()
        ...
        ### produce .dat file for Francesco (28 October 2015)
        datfileName = '/Users/gsavorgnan/galaxy_vivisection/results/tables/mass_sph_Francesco.txt'
        datfile = open(datfileName, 'w')
        datfile.write('# galaxy    MassSph     +err_MassSph     -err_MassSph   \n')
        for a,b,c,d in zip(gal_id, mass_sph, perr_mass_sph, merr_mass_sph):
        	datfile.write(str(a) + '    ' + str(b) + '    ' + str(c) + '    ' + str(d) +  '\n')
        datfile.close()
...
def inset_psb():
	...
	positive_offset = 0
	negative_offset = 0
	for ni, oi in zip(n,offset):
		if ni<2 and oi>0:
			positive_offset = positive_offset + 1
		elif ni<2 and oi<0:
			negative_offset = negative_offset + 1	
	print 'Among n<2 bulges, there are', positive_offset, 'with positive offset and', negative_offset, 'with negative offset.' 				
...
def mbh_vs_mag_tot():
	...
	ax.errorbar(mag_tot[morph_core==u'E_0'], mbh[morph_core==u'E_0'], 
        ...
        ax.errorbar(mag_tot[morph_core=='Sp_0'], mbh[morph_core=='Sp_0'], 
        	xerr=[err_mag_tot[morph_core=='Sp_0'],err_mag_tot[morph_core=='Sp_0']], 
        	yerr=[merr_mbh[morph_core=='Sp_0'],perr_mbh[morph_core=='Sp_0']], 
        	ecolor='blue', marker='s', mfc='blue', mec='blue', markersize=9, ls=' ', elinewidth=1.2, capthick=1.2, barsabove=False)
