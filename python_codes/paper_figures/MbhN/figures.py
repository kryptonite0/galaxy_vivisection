...
def mag_sph_vs_n_maj():
	...
	for x0,y0 in zip(n[simplemorphtype=='E'], mag_sph[simplemorphtype=='E']):
		markers_linlog.elliptical(ax, 'red', np.log10(x0), (y0), 0.035, 0.08)
	
        for x0,y0 in zip(n[simplemorphtype=='E/S0'], mag_sph[simplemorphtype=='E/S0']):
        	markers_linlog.lenticular(ax, 'red', np.log10(x0), (y0), 0.03, 0.1)
       
        for x0,y0 in zip(n[simplemorphtype=='S0'], mag_sph[simplemorphtype=='S0']):
        	markers_linlog.lenticular(ax, 'darkorange', np.log10(x0), (y0), 0.03, 0.1)
...
def mbh_vs_n_sph_maj():
	...
	#################################################
       ...
       #logxx = np.arange(-10,20,0.1)
       #yy = (A[2]*(logxx) + B[2])
       #ax.plot(10**(logxx+np.average(log_n[simplemorphtype=='S0'])),10**yy, color='b', ls='-', linewidth=2.)
       #
       ###### calculates 1sigma uncertainty band
       #yy_1 = ((A[2]+Aerr[2])*(logxx) + (B[2]+Berr[2]))
       #yy_2 = ((A[2]-Aerr[2])*(logxx) + (B[2]+Berr[2]))
       ...
       #yy_lo = np.asarray(yy_lo)
       #			
       #ax.fill_between(10**(logxx+np.average(log_n[simplemorphtype=='S0'])), 10**yy_lo, 10**yy_up, alpha=0.2, facecolor='b')
       ...
       #logxx = np.arange(-10,20,0.1)
       #yy = (A[2]*(logxx) + B[2])
       #ax.plot(10**(logxx+np.average(log_n[simplemorphtype=='E'])),10**yy, color='b', ls='-', linewidth=2.)
       #
       ###### calculates 1sigma uncertainty band
       #yy_1 = ((A[2]+Aerr[2])*(logxx) + (B[2]+Berr[2]))
       #yy_2 = ((A[2]-Aerr[2])*(logxx) + (B[2]+Berr[2]))
       ...
       #yy_lo = np.asarray(yy_lo)
       #			
       #ax.fill_between(10**(logxx+np.average(log_n[simplemorphtype=='E'])), 10**yy_lo, 10**yy_up, alpha=0.2, facecolor='b')
