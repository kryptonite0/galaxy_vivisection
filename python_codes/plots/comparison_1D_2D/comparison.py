...
def comparison_r_e():
	...
	ax.plot([0.1,1000], [0.1,1000], 'k--', linewidth=3)
        plt.xlabel(r'$R_{\rm e,sph}^{\rm 1D,maj} \rm~[arcsec]$', labelpad=30)
        plt.ylabel(r'$R_{\rm e,sph}^{\rm 2D,maj} \rm~[arcsec]$', labelpad=30)
...
def comparison_n():
	...
	ax.plot([0.1,100], [0.1,100], 'k--', linewidth=3)
        plt.xlabel(r'$n_{\rm sph}^{\rm 1D,eq}$', labelpad=30)
        plt.ylabel(r'$n_{\rm sph}^{\rm 2D}$', labelpad=30)
	plt.subplots_adjust(left=0.25,bottom=0.25)
...
def comparison_mag_sph():
	...
	#ax.set_yscale('log')
        ...
        #ax.set_yticks(ticks)
        #ax.set_yticklabels(ticks_labels)
	ax.axis([3.1,11.9,3.1,11.9])
	ax.plot([0.1,100], [0.1,100], 'k--', linewidth=3)
        plt.xlabel(r'$m_{\rm sph}^{\rm 1D} \rm~[mag]$', labelpad=30)
        plt.ylabel(r'$m_{\rm sph}^{\rm 2D} \rm~[mag]$', labelpad=30)
	plt.subplots_adjust(left=0.25,bottom=0.25)
