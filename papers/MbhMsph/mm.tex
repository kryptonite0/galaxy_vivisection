...
consistent with a dry-merging formation scenario,
and define a tight \emph{early-type sequence} with intrinsic scatter $\epsilon_{(Y|X)} = 0.43 \pm 0.06\rm~dex$. 
On the other hand, the bulges of the spiral galaxies trace a steeper \emph{late-type sequence}, 
...
\section{Conclusions}
Using $3.6\rm~\mu m$ \emph{Spitzer} imagery, 
...
and we have derived galaxy luminosities, spheroid luminosities and spheroid stellar masses for 66 nearby galaxies 
with a dynamical measurement of their black hole mass. 
Our galaxy sample, besides being the largest ever used to investigate black hole mass scaling relations, 
...
\item The $M_{\rm BH} - M_{\rm *,sph}$ log-relation for the spheroidal components of early-type (elliptical + lenticular) galaxies 
      ...
      The $M_{\rm BH} - M_{\rm *,sph}$ log-relation for the bulges of late-type (spiral) galaxies has a more uncertain slope ($2-3$) 
      due to the smaller size of the subsample and, more importantly, to the smaller range in stellar mass 
      ...
      it would be tempting to refer to our \emph{early-type sequence} as a \emph{red sequence}, 
      although no bimodality in color has been observed in the $M_{\rm BH} - M_{\rm *,sph}$ diagram.
\item The $M_{\rm BH} - M_{\rm *,sph}$ log-relations for core-S\'ersic and S\'ersic spheroids have slopes consistent with each other 
      ...
      thus these two populations do not define two distinct sequences. 
\item In the $M_{\rm BH} - L_{\rm sph}$ (or $M_{\rm BH} - M_{\rm *,sph}$) diagram, for early-type galaxies, 
      we did not observe the change in slope required for consistency with the $M_{\rm BH} - \sigma$ and bent $L_{\rm sph} - \sigma$ correlations.
      This issue of inconsistency remains therefore an open question. 
      It might be that we are still not probing enough low black hole masses ($M_{\rm BH} < 10^7\rm~M_\odot$) 
      for the subsample of early-type galaxies, 
      or that the transition from  $L_{\rm sph} \propto \sigma^2$ at low luminosities to $L_{\rm sph} \propto \sigma^5$ at high luminosities 
...
\acknowledgments
GS warmly thanks Luca Cortese, Elisabete Lima Da Cunha, Duncan Forbes, Gonzalo Diaz and Pablo Rosado for useful discussion. \\
% referee thank you!!
