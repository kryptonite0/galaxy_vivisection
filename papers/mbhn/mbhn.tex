...
\author{G. A. D. Savorgnan\altaffilmark{1}}
...
we performed state-of-the-art modelling for the largest sample of galaxies to date (Savorgnan \& Graham 2015a, hereafter \emph{Paper I}),  
...
In Savorgnan et al.~(2015, hereafter \emph{Paper II}) we examined the correlations between the black hole mass and 
...
Savorgnan \& Graham (2015b) demonstrate that when an intermediate-scale disk is misclassified and modeled as a large-scale disk, 
...
{\bf quote fitexy results bcz more reliable than other 2 }
%I should have mentioned that the linear regression code, BCES, from Akritas & Bershady (1996) can at times get the slope wrong.  
%This occurs if the error bars on the velocity dispersion are too big relative to the range of velocity dispersions in one's sample (say >15%), 
%or if the error bars on the velocity dispersion and black hole masses are not all roughly the same. 
%That is, if some black hole masses have error bars of 50% while others have error bars of 5%, then we could be in trouble.
...
Early-type galaxies follow $L_{\rm sph} \propto n_{\rm sph}^{3.60 \pm 0.19}$; 
remarkably, this is exactly the same proportionality obtained by \cite{savorgnan2013} for elliptical galaxies only, 
upon re-analyzing the dataset from \cite{grahamguzman2003}. 
Late-type galaxies follow a shallower $L_{\rm sph} \propto n_{\rm sph}^{1.44 \pm 0.52}$ sequence. 
...
\begin{equation*}
\log \biggl( \frac{M_{\rm BH}}{\rm M_\odot} \biggr) = (8.15 \pm 0.06) + (3.37 \pm 0.15) \bigl(\log n_{\rm sph}^{\rm maj} - 0.50 \bigr) . 
\end{equation*}
\\
...
\\
 ...
 All (64)		& BCES $(Y|X)$  	     & $8.14 \pm 0.07$ & $3.49 \pm 0.36$ & $0.50$ & $-$ & $0.61$ \\
 			...
 			& mFITEXY Bisector	     & $8.15 \pm 0.06$ & $3.37 \pm 0.15$ & $0.50$ & $-$ & $0.59$ \\
