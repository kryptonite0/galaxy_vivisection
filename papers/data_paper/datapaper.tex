...
Interestingly, the past studies all %investigated the black hole mass scaling relations using 
used almost the same sample of galaxies, 
but they claimed some contradictory conclusions.
One study \citep{grahamdriver2007} obtained a good $M_{\rm BH}-n_{\rm sph}$ correlation
(the bulge S\'ersic index $n_{\rm sph}$ is a measure of the central radial concentration of stars), 
whereas the remaining four did not.
\cite{lasker2014anal} declared that $M_{\rm BH}$ correlates equally well with the total galaxy luminosity and the bulge luminosity, 
...
We will use these to obtain improved black hole mass scaling relations using the largest to date sample of 72 galaxies.

This paper is structured as follows.
Section \ref{sec:data} presents the data-set used to conduct this study. 
...
In Section \ref{sec:res} we outline the results from our analysis 
and we discuss the error analysis. 
...
has been reported in the literature and for which at least one $3.6~\rm \mu m$ \emph{Spitzer}/IRAC observation was made publicy 
available at the time we began our investigation (2013).
Black hole masses were drawn from the catalog of \citet{grahamscott2013} for 70 galaxies,
from \citet{rusli2013bhmassesDM} for 4 galaxies 
and from \citet{greenhill2003} for 1 galaxy. 
...
We performed image mosaicing using the %(command line version of the) 
...
\caption{An example of the \emph{overlap} correction. 
...
{\tt objmasks}, %\footnote{{\tt objmasks} is part of the {\tt noao.nproto} package.},
...
concerns the kinematic properties of galaxies within the instrument field-of-view, % (typically up to one effective radius),
...
of galaxies.
%This is naively identified with the whole galaxy in case of an elliptical galaxy,
%and with the bulge of a lenticular or spiral galaxy.
...
%In Figures \ref{fig:re1d2d}, \ref{fig:n1d2d} and \ref{fig:mag1d2d}, we compare the bulge best-fit parameters obtained 
%from the 1D decompositions with those obtained from the 2D decompositions.
...
%(if no weighting scheme is applied to the data points during the minimization routine, 
%then, for the same galaxy, the fit of a logarithmically sampled light profile will naturally put more weight on the inner part of the profile 
%than in the case of linear sampling).
...
%The absolute values of the uncertainties of the S\'ersic parameters will be estimated in a forthcoming paper, 
%according to the grades assigned here to the fits.
...
% This galaxy has also a partially depleted core ($R_{\rm break} = 0''.69$, \citealt{rusli2013}).
%
% %\cite{sani2011} fit this galaxy with a S\'ersic-bulge + Gaussian-AGN model ($R_{\rm e} = 61''.1$, $n=6$).
% %\cite{lasker2014data} used a S\'ersic-bulge + mask-core model ($R_{\rm e} = 62''.4$, $n=8.25$).
% %\cite{rusli2013} adopted a core-S\'ersic model ($R_{\rm e} = 45''.43$, $n=7.6$).
%
% \emph{1D fit -- }
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for IC 1459.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
%   &  &  $[\rm arcsec]$ & \\
% \hline
...
%
% %\cite{sani2011} used a S\'ersic-bulge + exponential-disk + Gaussian-AGN model ($R_{\rm e} = 27''.5$, $n=2$).
%
% \emph{1D fit -- }
% A visual inspection of the image of IC 2560 reveals a boxy bulge and
% a large scale bar that extends out to $R \lesssim 43''$.
% The disk appears to be slightly lopsided along the direction of the bar,
% due to two non-symmetric ansae, but it becomes symmetric beyond $R \gtrsim 60''$. 
% This is why the surface brightness profile deviates from a perfect exponential
% in the radial range $42'' \lesssim R \lesssim 60''$, which has been excluded from the fit.
...
% %Our final 1D model consists of a S\'ersic-bulge + exponential-disk + Ferrer-bar + Gaussian-AGN.
% %Similarly, our 2D model consists of a S\'ersic-bulge + exponential-disk + Gaussian-bar + Gaussian-AGN.\\
%
% \begin{figure}[h]
% \begin{center}
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for IC 2560.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% {\bf IC 4296 \\}
%
% IC 4296 is an elliptical galaxy. % with a radio jet \citep{liuxie1992}.
% Due to its high stellar velocity dispersion, this galaxy is expected to host a partially depleted core. 
%
% %\cite{sani2011} fit this galaxy with a S\'ersic-bulge + Gaussian-AGN model ($R_{\rm e} = 33''.6$, $n = 4$).
% %\cite{lasker2014data} employed a S\'ersic-bulge + mask-core model ($R_{\rm e} = 97''.8$, $n=8.2$).
%
% \emph{1D fit -- }
% After masking the innermost $6''.1$, % affected by the (unresolved) partially depleted core,
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for IC 4296.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% M31 also features a pseudo-ring-like structure at $R \sim 50'$ \citep{athanassoulabeaton2006m31}. 
%
%
% \emph{1D fit -- }
% We applied the \emph{smoothing} technique described in Section \ref{sec:smooth} to the analysis of M31.
% The region $2000'' \lesssim R \lesssim 3400''$, where the pseudo-ring is observed, was excluded from the fit.
% A S\'ersic + exponential fit is clearly not adequate to describe the light profile of M31.
% The residuals of such fit display a structure in correspondence of the bar ($R \lesssim 1500''$).
% The bar was thus fit with a Ferrer function.
%
%
% \begin{figure}[h]
% \begin{center}
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for M31.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% with a partially depleted core ($R_{\rm break} = 1''.8$, \citealt{rusli2013}).
% %M49 is a slow-rotator \citep{atlas3dIII-MNRAS}.
%
% %\cite{ferrarese2006acsvcs} modeled this galaxy with a core-S\'ersic profile ($R_{\rm e} = 208''$, $n = 5.5$).
% %Also \cite{rusli2013} used a core-S\'ersic model ($R_{\rm e} = 199''$, $n = 5.6$).
%
% \emph{1D fit -- }
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for M49.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% %M59 is a fast-rotator \citep{atlas3dIII-MNRAS}.
%
% %\cite{sani2011} modeled this galaxy with a single S\'ersic-bulge model ($R_{\rm e} = 70''.1$, $n = 5$).
% %\cite{vika2012} used a S\'ersic-bulge + mask-core model ($R_{\rm e} = 54''.7$, $n = 5.7$). 
%
% \emph{1D fit -- }
...
% Although faint, it is important to account for the presence of this disk.
% A single S\'ersic fit produces bad residuals.
% The fit notably improves if an embedded exponential disk function 
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for M59.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
%   &  &  $[\rm arcsec]$ & \\
% \hline
% T.W. 1D maj. & S-bul + m-c + e-id & $48.0$  &  $5.5$ \\
% T.W. 1D eq.  & S-bul + m-c + e-id & $90.9$  &  $8.8$ \\
% %T.W. 2D	&    & $$  &  $$ \\
% \hline 
% S+11         & S-bul       & $70.1$  &  $5.0$ \\
...
%
% %B12 model the galaxy image with a 2D S\'ersic function + exponential disk ($n = 1.49$). \\
% \emph{1D fit -- }
% A large-scale bar ($R \lesssim 50''$) can be identified in the unsharp mask 
% and produces corresponding peaks in the ellipticity, PA and B4 profiles.
% We modeled it with a Ferrer function.
% Additional peaks in the ellipticity, PA and B4 profiles at $R \sim 15''$ 
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for M64.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% {\bf M81 -- NGC 3031 \\}
%
% M81 is an early-type spiral galaxy with a LINER AGN.
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for M81.}
% \begin{center}
% \begin{tabular}{llccc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% {\bf M84 -- NGC 4374 \\}
%
% M84 is an elliptical galaxy with a partially depleted core ($R_{\rm break} = 1''.5$, \citealt{rusli2013}). 
%
% %GD07 model the galaxy light profile with a S\'ersic function ($n = 5.6$).
% %V12 model the galaxy image with a 2D S\'ersic function + core-mask ($n = 3.5$).
% %S11 model the galaxy image with a 2D S\'ersic function + Gaussian ($n = 7.0$).
% %B12 model the galaxy image with a 2D S\'ersic function ($n = 4.10$). \\
%
% \emph{1D fit -- }
% The unsharp mask reveals a faint inner component ($R \lesssim 12''$). 
% Here we excluded the data within $R < 13''.8$ and modeled the galaxy with a S\'ersic profile.
%
% %After masking the innermost $13''.8$, we modeled the galaxy with a S\'ersic profile. 
% %However, we notice that 
% %the 1D and 2D fits are not much affected by the presence of the embedded component.
% %In fact, reducing the size of the central mask to uncover the inner component 
% %(but still masking the region affected by the partially depleted core) does not
% %alter significantly the bulge parameters. 
% %The structures observed in the residuals are caused by the negative ellipticity gradient 
% %(from $\epsilon \sim 0.17$ to $\epsilon \sim 0.03$, over a radial range $10'' < R < 130''$),
% %which cannot be reproduced by the 2D model because of its fixed ellipticity ($\epsilon = 0.10$). 
%   
%
% \begin{figure}[h]
% \begin{center}
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for M84.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% This galaxy has a large partially depleted core ($R_{\rm break} = 7''.2$, \citealt{ferrarese2006acsvcs}).
%
% \emph{1D fit -- }
% The morphology of M87 is similar to that of M49.
% The innermost $18''.5$ were excluded from the fit.
% We used a single S\'ersic model to characterize M87.
%
%
% \begin{figure}[h]
% \begin{center}
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for M87.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for M89.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% This galaxy has a ring-like structure at $R \sim 50''$ \citep{munoztunon1989m94}.
%
% %\cite{fisherdrory2010} fit M94 with a S\'ersic-bulge + exponential-disk model ($R_{\rm e} = 7''.7$, $n = 1.3$). 
%
% \emph{1D fit -- }
% The ellipticity and B4 profiles display a local maximum at $R \sim 10''$, 
...
% Therefore we preferred to exclude also the data in the range $20'' \lesssim R \lesssim 80''$, 
% where the ring light dominates over the rest of the galaxy.
% %Our final model for M94 consists of a S\'ersic-bulge + exponential-disk + Ferrer-bar.
%
% \begin{figure}[h]
% \begin{center}
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for M94.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
%
% %\cite{sani2011} fit this galaxy with a S\'ersic-bulge + exponential-disk + Gaussian-bar model 
% %($R_{\rm e} = 45''.6$, $n = 1.0$). 
% \cite{nowak2010n3368n3489} argued for the presence of a large pseudo-bulge and a tiny classical bulge 
% ($R_{\rm e} = 1''.6$, $n = 2.3$). 
%
% \emph{1D fit -- }
% M96 has a complex morphology. 
% The large-scale disk does not exhibit a perfect exponential profile, 
% mainly because of wound spiral arms resembling a ring ($R \sim 185''$),
% which however are too faint and irregular to be described with a Gaussian ring model. 
% The large scale bar extends out to $R \lesssim 80''$ and has been modeled with a Ferrer function.
% A peak at $R \sim 25''$ in the ellipticity profile unravels an inner component embedded in the bulge.
% The isophotal twist observed within $R \lesssim 25''$ suggests that the inner component can be identified 
% with a second bar, as already noted by \cite{erwin2004}.
% We accounted for this embedded component with a low-$n$ S\'ersic profile.
% We built our final model with a S\'ersic-bulge + exponential-disk + Ferrer-bar + S\'ersic-bar.
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for M96.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for M105.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% with %a radio jet \citep{liuzhang2002} and 
% circumnuclear dust \citep{martini2003}. 
%
% %\cite{grahamdriver2007} modeled the galaxy light profile with a S\'ersic-bulge + exponential-disk 
% %($R_{\rm e} = 14''.9$, $n = 2.0$). 
% %\cite{sani2011} used a S\'ersic-bulge + exponential-disk + Gaussian-AGN model 
% %($R_{\rm e} = 111''.7$, $n = 2.0$). 
% %\cite{vika2012} proposed two different fits: the first with a S\'ersic-bulge + exponential-disk + PSF-nucleus
% %($R_{\rm e} = 17''.0$, $n = 3.5$), 
% %and the second with a S\'ersic-bulge + exponential-disk + PSF-nucleus + S\'ersic-bar
% %($R_{\rm e} = 6''.3$, $n = 2.2$).
%
% \emph{1D fit -- }
...
% %Our final model consists of a S\'ersic-bulge + exponential-disk.
%
% \begin{figure}[h]
% \begin{center}
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for M106.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
%
% %\cite{laurikainen2010} modeled this galaxy with a S\'ersic-bulge + exponential-disk + 2 Ferrer-lenses 
% %($R_{\rm e} = 8''.9$, $n = 2.7$).
% %\cite{sani2011} opted for a S\'ersic-bulge + exponential-disk 
% %($R_{\rm e} = 26''.8$, $n = 3.0$).
% \emph{1D fit -- }
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 0524.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% {\bf NGC 1023 \\}
%
% NGC 1023 is a barred lenticular galaxy. % with no partially depleted core \cite{faber1997}. \\
%
% %\cite{grahamdriver2007} modeled the galaxy light profile with a S\'ersic-bulge + exponential-disk 
% %($R_{\rm e} = 17''.7$, $n = 2.0$). 
% %\cite{sani2011} used a S\'ersic-bulge + exponential-disk + Gaussian-bar model 
% %($R_{\rm e} = 24''.0$, $n = 3.0$).
% %\cite{lasker2014data} employed a S\'ersic-bulge + exponential-disk + S\'ersic-bar 
% %($R_{\rm e} = 9''.6$, $n = 3.1$).
% \emph{1D fit -- }
% The bar extends out to $R \lesssim 40''$.
% %The light profile of NGC 1023 is well described by a S\'ersic-bulge + exponential-disk + Ferrer-bar model.
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 1023.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% {\bf NGC 1300 \\}
%
% NGC 1300 is a face-on, barred spiral galaxy. % with no partially depleted core \citep{graham2012MNRAS}. 
%
% %\cite{sani2011} fit NGC 1300 with a S\'ersic-bulge + exponential-disk 
% %($R_{\rm e} = 85''.4$, $n = 3.0$).
% %\cite{lasker2014data} built their model with a S\'ersic-bulge + exponential-disk + PSF-nucleus + exponential-inner-disk + 
% %S\'ersic-bar + spiral arms
% %($R_{\rm e} = 10''.4$, $n = 4.3$).
%
% \emph{1D fit -- }
...
% %Our final model consists of a S\'ersic-bulge + exponential-(disk+bar) + S\'ersic-inner disk.
%
% \begin{figure}[h]
% \begin{center}
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 1300.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 1316.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 1332.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
%
% %\cite{grahamdriver2007} fit the galaxy light profile with a single S\'ersic-bulge profile 
% %($n = 16.8$).  
% %\cite{rusli2013} modeled NGC 1399 with a core-S\'ersic profile and the sum of a S\'ersic and an exponential
% %function accounting for the galaxy halo 
% %($R_{\rm e} = 36''.2$, $n = 7.38$). 
% %\cite{lasker2014data} used a S\'ersic-bulge + mask-core model 
% %($R_{\rm e} = 154''.0$, $n = 11.1$). 
%
% \emph{1D fit -- }
...
% %We build our final model with a S\'ersic-bulge + mask-core + exponential-inner disk.
%
% \begin{figure}[h]
% \begin{center}
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 1399.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% and the inner disk. 
% %Their final model consists of a S\'ersic-bulge + exponential-disk + Ferrer-bar 
% %($R_{\rm e} = 2''.6$, $n = 1.8$). 
%
% \emph{1D fit -- }
...
% %Our final model consists of a S\'ersic-bulge + exponential-disk + Ferrer-bar.
%
% \begin{figure}[h]
% \begin{center}
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 2273.}
% \begin{center}
% \begin{tabular}{llccc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% %We build our final model with a S\'ersic-bulge + exponential-disk + 2 Ferrer-bars.
%
% \begin{figure}[h]
% \begin{center}
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 2549.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
%
% %\cite{grahamdriver2007} modeled NGC 2778 with a S\'ersic-bulge + exponential-disk 
% %($R_{\rm e} = 2''.3$, $n = 1.6$). 
% %\cite{sani2011} used a S\'ersic-bulge + exponential-disk model
% %($R_{\rm e} = 2''.5$, $n = 2.5$). 
% %\cite{vika2012} employed a S\'ersic-bulge + exponential-disk model 
% %($R_{\rm e} = 1''.5$, $n = 2.7$). 
% %\cite{lasker2014data} fit the galaxy with a S\'ersic-bulge + exponential-disk + S\'ersic-bar
% %($R_{\rm e} = 2''.8$, $n = 4.0$). 
%
% \emph{1D fit -- }
...
% %We then fit NGC 2778 with a S\'ersic-bulge + exponential-disk + Ferrer-bar + PSF-nucleus.
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 2778.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% %\cite{grahamdriver2007} modeled the galaxy light profile with a S\'ersic-bulge + exponential-disk 
% %($R_{\rm e} = 4''.6$, $n = 2.0$). 
% %\cite{fisherdrory2008} decomposed NGC 2787 in a S\'ersic-bulge + exponential-disk 
% %($R_{\rm e} = 9''.1$, $n = 1.2$).
% %\cite{laurikainen2010} employed a S\'ersic-bulge + exponential-disk + Ferrer-bar + Ferrer-lens model
% %($R_{\rm e} \sim 4''$, $n = 1.3$).
% %\cite{sani2011} used a S\'ersic-bulge + exponential-disk + Gaussian-bar + Gaussian-AGN 
% %($R_{\rm e} = 15''.7$, $n = 3.0$). 
% %\cite{lasker2014data} described NGC 2787 with a S\'ersic-bulge + truncated exponential-disk + 
% %truncated S\'ersic-bar + S\'ersic-inner disk + PSF-nucleus
% %($R_{\rm e} = 14''.3$, $n = 2.8$). 
%
% \emph{1D fit -- }
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 2787.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 2974.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
%
% %\cite{rusli2013} modeled NGC 3091 with a single core-S\'ersic profile 
% %($R_{\rm e} = 91''.0$, $n = 9.3$). 
%
% \emph{1D fit -- }
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 3091.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% ($R_{\rm e} = 31''.0$, $n = 3.9$). 
% %\cite{sani2011} modeled NGC 3115 with a S\'ersic-bulge + exponential disk 
% %($R_{\rm e} = 27''.3$, $n = 3.0$). 
% %\cite{lasker2014data} used a S\'ersic-bulge + exponential disk + S\'ersic-envelope 
% %($R_{\rm e} = 3''.9$, $n = 3.0$). 
%
% \emph{1D fit -- }
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 3115.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
%
% %\cite{laurikainen2004} decomposed NGC 3227 into a S\'ersic-bulge + exponential-disk + Ferrer-bar 
% %($R_{\rm e} = 1''.8$, $n = 2.2$). 
% %\cite{sani2011} described this galaxy with a S\'ersic-bulge + exponential-disk 
% %($R_{\rm e} = 82''.9$, $n = 4.0$). 
% %\cite{lasker2014data} used a S\'ersic-bulge + exponential disk + S\'ersic-bar 
% %($R_{\rm e} = 0''.7$, $n = 4.1$). 
%
% \emph{1D fit -- }
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 3227.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
%
% %\cite{grahamdriver2007} modeled this galaxy with a S\'ersic-bulge + exponential-disk 
% %($R_{\rm e} = 11''.3$, $n = 4.3$). 
% %\cite{laurikainen2010} described the galaxy with a S\'ersic-bulge + exponential-(disk+lens) + Ferrer-inner disk 
% %($R_{\rm e} \sim 4''$, $n = 2.4$).  
% %\cite{sani2011} used a S\'ersic-bulge + exponential-disk 
% %($R_{\rm e} = 4''.6$, $n = 2.5$). 
% %\cite{vika2012} employed a S\'ersic-bulge + exponential-disk 
% %($R_{\rm e} = 3''.5$, $n = 2.6$). 
% %\cite{beifiori2012} decomposed the galaxy into a S\'ersic-bulge + exponential-disk 
% %($R_{\rm e} = 4''.00$, $n = 1.6$). 
% %\cite{lasker2014data} built their model with a S\'ersic-bulge + exponential-disk + S\'ersic-bar 
% %($R_{\rm e} = 1''.95$, $n = 1.6$). 
%
% \emph{1D fit -- }
...
% %We model the galaxy with a S\'ersic-bulge + exponential-disk + Ferrer-inner disk.
%
% \begin{figure}[h]
% \begin{center}
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 3245.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
%
% %\cite{grahamdriver2007} fit this galaxy with a S\'ersic-bulge + exponential-disk 
% %($R_{\rm e} = 2''.5$, $n = 1.7$). 
% %\cite{laurikainen2010} described the galaxy with a S\'ersic-bulge + exponential-disk + 2 Ferrer-bars
% %($R_{\rm e} \sim 4''$, $n = 1.5$).  
% %\cite{sani2011} employed a S\'ersic-bulge + exponential-disk + Gaussian-bar 
% %($R_{\rm e} = 4''.4$, $n = 2.5$). 
% %\cite{beifiori2012} modeled the galaxy with a S\'ersic-bulge + exponential-disk 
% %($R_{\rm e} = 8''.3$, $n = 2.3$). 
% %\cite{lasker2014data} decomposed the galaxy into a S\'ersic-bulge + exponential-disk + S\'ersic-inner disk + 
% %S\'ersic-nuclear disk + S\'ersic-bar
% %($R_{\rm e} = 5''.9$, $n = 2.5$). 
%
% \emph{1D fit -- }
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 3384.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 3393.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 3414.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% %\cite{laurikainen2010} fit this galaxy with a S\'ersic-bulge + exponential-disk + Ferrer-bar
% %($R_{\rm e} \sim 2''$, $n = 2.1$). 
% %\cite{sani2011} used a S\'ersic-bulge + exponential-disk 
% %($R_{\rm e} = 4''.6$, $n = 1.5$). 
%
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 3489.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
%
% %\cite{rusli2013} modeled this galaxy with a core-S\'ersic model 
% %($R_{\rm e} = 58''.8$, $n = 6.3$). 
%
% \emph{1D fit -- }
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 3842.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% %($R_{\rm e} \sim 5''$, $n = 2.0$).
% %\cite{sani2011} decomposed this galaxy into a S\'ersic-bulge + exponential-disk + Gaussian-bar + Gaussian-AGN 
% %($R_{\rm e} = 4''.7$, $n = 1.5$).
% %\cite{beifiori2012} modeled this galaxy with a S\'ersic-bulge + exponential-disk 
% %($R_{\rm e} = 5''.7$, $n = 2.3$).
% %\cite{lasker2014data} used a S\'ersic-bulge + exponential-disk (with inner truncation) + PSF-nucleus + 
% %S\'ersic-inner disk + S\'ersic-bar 
% %($R_{\rm e} = 2''.0$, $n = 1.1$).
%
% \emph{1D fit -- }
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 3998.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 4026.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% {\bf NGC 4261 \\}
%
% NGC 4261 is an elliptical galaxy with %a radio jet \citep{liuxie1992}, 
...
%
% %\cite{grahamdriver2007} fit NGC 4261 with a single S\'ersic-bulge 
% %($R_{\rm e} = 88''.6$, $n = 7.3$).
% %\cite{sani2011} used a S\'ersic-bulge + exponential-disk + Gaussian-AGN model 
% %($R_{\rm e} = 22''.6$, $n = 4.0$).
% %\cite{vika2012} employed a S\'ersic-bulge + mask-core 
% %($R_{\rm e} = 24''.2$, $n = 3.5$).
% %\cite{beifiori2012} adopted a single S\'ersic-bulge model 
% %($R_{\rm e} = 48''.8$, $n = 4.3$).
% %\cite{rusli2013} modeled this galaxy with a core-S\'ersic profile 
% %($R_{\rm e} = 77'.1$, $n = 6.3$).
% %\cite{lasker2014data} employed a S\'ersic-bulge + mask-core model 
% %($R_{\rm e} = 68'.4$, $n = 6.5$).
%
% \emph{1D fit -- }
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 4261.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% %\cite{rusli2013} modeled this galaxy with a core-S\'ersic profile 
% %($R_{\rm e} = 15''.3$, $n = 5.6$).
% %\cite{lasker2014data} adopted a S\'ersic-bulge + mask-core model
% %($R_{\rm e} = 21''.3$, $n = 8.6$).
%
% \emph{1D fit -- }
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 4291.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 4388.}
% \begin{center}
% \begin{tabular}{llccc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
%
% %\cite{laurikainen2010} decomposed this galaxy into a S\'ersic-bulge + exponential-disk + Ferrer-lens 
% %($R_{\rm e} \sim 7'' $, $n = 3.0$).
% %\cite{sani2011} used a S\'ersic-bulge + exponential-disk 
% %($R_{\rm e} = 10''.3$, $n = 2.5$).
% %\cite{vika2012} modeled this galaxy with a S\'ersic-bulge + Moffat-nucleus 
% %($R_{\rm e} = 25''.0$, $n = 3.9$).
% %\cite{beifiori2012} adopted a single S\'ersic-bulge model 
% %($R_{\rm e} = 155''.2$, $n = 7.4$).
%
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 4459.}
% \begin{center}
% \begin{tabular}{llccc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 4564.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 4596.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 4697.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 4889.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 4945.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 5077.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 5128.}
% \begin{center}
% \begin{tabular}{llccc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 5845.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 5846.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 6251.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 7052.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 7619.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for NGC 7768.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
% \begin{table}[h]
% \small
% \caption{Best-fit parameters for UGC 03789.}
% \begin{center}
% \begin{tabular}{llcc}
% \hline
% {\bf Work} & {\bf Model}   & $\bm R_{\rm e}$    & $\bm n$ \\
...
\bibliography{/Users/gsavorgnan/Dropbox/giulia_e_basta/Literature/SMBHbibliography}
