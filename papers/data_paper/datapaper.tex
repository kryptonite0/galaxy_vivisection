...
%\clearpage
  ...
  NGC 3377 is an elliptical/lenticular galaxy with an intermediate-scale disk and a nuclear stellar disk. 
  }
  \end{center}
  \end{figure}
  ...
  The morphology of NGC3377 is similar to NGC0821. 
  The unsharp mask reveals an embedded disc and the extended velocity field confirms that 
  it is not a large-scale disc. 
  The residual image from a 2D single S\'ersic fit diplays the unsubtracted embedded disc
  and an additional nuclear ($\lesssim 10''$) component.
  The embedded disc is very close to edge-on.
  In the 2D fit we adopt a S\'ersic + edge-on disc model, after masking the third nuclear component. 
  In the 1D fit we are not able to model the embedded disc with an exponential function, 
  probably because of the orientation of the disc. 
  The best results are obtained when a Ferrer function is employed. 
  We tried to model the nuclear cusp with a PSF or a gaussian function. The gaussian function parameters
  resulted highly degenerate with the Ferrer function parameters, leading us to prefer a PSF profile. \\
