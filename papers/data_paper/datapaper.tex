...
\title{Supermassive black hole and host bulge affairs \\ I. True to life, physically motivated galaxy decomposition}
...
to investigate correlations between the black hole mass and the host spheroid properties, 
but they have not converged to the same conclusions. 
This is because their best fit models for the same galaxy were often significantly different 
...
Thanks to a scrupulous inspection of the galaxy photometry (through isophotal analysis and unsharp masking) and -- for the first time -- of the 2D kinematics, 
we were able to identify and model bulges, large-scale, intermediate-scale or nuclear disks, bars, rings, spiral arms, halos, extended or unresolved nuclear 
sources and partially depleted cores. 
For each individual galaxy, we compared our best-fit model with those presented by the previous studies, 
explained the discrepancies and identified the optimal decomposition.
We independently performed 1D and 2D decompositions, 
and concluded that, at least when modelling large, nearby galaxies, 1D techniques have more advantages than 2D techniques. 
Finally, we developed a method to estimate the uncertainties on the 1D best-fit parameters 
...
\clearpage
  ...
  M104 is a lenticular/spiral galaxy with a partially depleted core. 
  The data within the innermost $6''.1$ are excluded from the fit, 
  and the most obvious ring of the galaxy is modeled.
  }
  \end{center}
  \end{figure}
  ...
  {\bf Work} & {\bf Model}   & $\bm R_{\rm e,sph}$    & $\bm n_{\rm sph}$ \\
    &  &  $[\rm arcsec]$ & \\
  \hline
  T.W. 1D maj. & S-bul + e-d + G-r + m-c  & $11.1$  &  $5.4$ \\
  T.W. 1D eq.  & S-bul + e-d + G-r + m-c  & $52.5$  &  $7.1$ \\
  \hline
  S+11         & S-bul + e-d + G-bar + G-n  & $66.1$  &  $1.5$ \\
  \hline
  \end{tabular}
  \end{center}
  \label{tab:m104}
  \tablecomments{The bulge S\'ersic index obtained by S+11 is smaller than our estimates because 
  they fit a nuclear component. }
  \end{table}
  ...
  which is at odds with the fact that central cluster galaxies typically have very large sizes. 
...
%  }
  ...
  \end{center}
  \label{tab:n7052}
  \tablecomments{The model of V+12 oversubtracts a large-scale disk and thus results in the lowest estimates of the effective radius and S\'ersic index.}
  \end{table}
